...
$user = $_GET['uid'];//提交的用户  也可以去session中取登录用户 $_SESSION['uid']

$pay_id = $user; //网站唯一标识 需要充值的用户名，用户ID或者自行创建订单 建议传递用户的ID

$price = 0.01; //提交的价格

$param = $_GET['vid']; //自定义参数  可以留空 传递什么返回什么 用于区分游戏分区或用户身份
...
$parameter = array(
    ...
    "return_url" => "http://aishan.clwub.xin/index/single/getpay",//付款后附带加密参数跳转到该页面
