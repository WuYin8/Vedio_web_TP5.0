<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:76:"/home/wwwroot/aishan.clwub.xin/public/../application/admin/view/res/res.html";i:1519796864;}*/ ?>
<!DOCTYPE html>
...
<!--heder end here-->
	...
	<ol class="breadcrumb">
               <li class="breadcrumb-item"><a href="">视频基本信息</a></li>
            </ol>
...
<!-- tab content -->
	<div class="container">
                ...
                <table class="table table-hover" style="font-weight: bold;margin-left: -20px;width:1000px;">
	                ...
	                <thead>
	                	<tr>
	                		<th></th><th>ID</th><th>头像</th><th>姓名</th><th>标题</th><th>视频简介</th><th>标签</th><th>时长</th><th>评论量</th><th>点击量</th><th>状态</th><th>操作1</th><th>操作2</th><th>操作3</th>
	                	</tr>
	                </thead>
  				<tbody>
  					...
  					<tr>
		                ...
		                <td><img src="<?php echo $res['up_photo']; ?>" style="width:20px;height:20px"></td>
		                <td><?php echo $res['up_name']; ?></td>
		                <td><a href="/index/single/single?vid=<?php echo $res['id']; ?>"><?php echo $res['title']; ?></td>
		                <td><?php echo $res['content']; ?></td>

		                <td><?php echo $res['label']; ?></td>
		                  <td><?php echo $res['time_long']; ?></td>
		               
		               
		                <td><?php echo $res['report_num']; ?></td>
		                <td><?php echo $res['click_num']; ?></td>
		                <td>
		                 <?php if($res['is_pass']==1): ?>
		                 已审核
		                 <?php else: ?>
		                 未审核
		                 <?php endif; ?>
		                ...
		                <td><a href="#" onclick="doDelete(<?php echo $res['id']; ?>)"   class="btn btn-link del"  type="button">回收站</a></td>
		                <td><a href="#" onclick="deletes(<?php echo $res['id']; ?>)"  class="btn btn-link del" type="button">删除</a></td> 
		                <td><a href="#" onclick="payRes(<?php echo $res['id']; ?>)"  class="btn btn-link del" type="button">
		                	<?php if($res['is_pay']==1): ?>
			                 取消付费
			                 <?php else: ?>
			                 设为付费
	...
	<!-- 设为收费 -->
           ...
           function payRes(id){
           	...
           	if(confirm('确认要修改视频付费模式吗!'))
           	{
           		$.ajax({
           			type:'get',
           			url:'/admin/res/changePay',
           			data:{id:id},
           			error:error,
           			success:success
